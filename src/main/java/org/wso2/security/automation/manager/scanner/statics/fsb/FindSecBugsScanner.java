package org.wso2.security.automation.manager.scanner.statics.fsb;/*
...
import org.wso2.security.automation.manager.handler.ServerHandler;
...
public class FindSecBugsScanner implements StaticScanner {
    ...
    public FindSecBugsScanner() {
        staticScanner = new FindSecBugsEntity();
    ...
    public StaticScannerEntity startContainer() {
        ...
        staticScanner.setStatus(ScannerProperty.getStatusInitiated());
        staticScannerService.save(staticScanner);

        int port = StaticScanner.calculatePort(staticScanner.getId());

        String containerId = DockerHandler.createContainer(ScannerProperty.getFindsecbugsScannerDockerImage(), ipAddress, String.valueOf(port),
        ...
        if (containerId != null) {
            ...
            staticScanner.setStatus(ScannerProperty.getStatusCreated());
            staticScanner.setCreatedTime(createdTime);

            staticScannerService.save(staticScanner);
            if (DockerHandler.startContainer(containerId)) {
                staticScanner.setStatus(ScannerProperty.getStatusRunning());
