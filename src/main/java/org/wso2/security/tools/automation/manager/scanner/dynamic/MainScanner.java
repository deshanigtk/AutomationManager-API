        ...
        if (dynamicScannerEntity != null) {
            if (true) {
                ...
                if (productManagerEntity != null) {
                    if (ServerHandler.hostAvailabilityCheck(productManagerEntity.getIpAddress(), productManagerEntity.getHostPort(), 12 * 3)) {
                        if (productManager.startWso2Server()) {
                            if (ServerHandler.hostAvailabilityCheck(productManagerEntity.getIpAddress(), 9443, 12 * 5)) {
                                dynamicScanner.startScan(dynamicScannerEntity, productManagerEntity);
//                                dynamicScanner.getReportAndMail(dynamicScannerEntity.getContainerId(), fileUploadLocation + File.separator + ScannerProperties.getZapReport());
                            }
                        }
                    }
                }
